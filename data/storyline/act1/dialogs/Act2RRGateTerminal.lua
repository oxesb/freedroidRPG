...
return {
	EveryTime = function()
		...
		RRGateTerminal_date_1 = os.date("%a %b %d %H:%M:%S") -- emulate os.date() but without the year --  can we translate this?
		...
		if (cmp_obstacle_state("Act2CityGate", "closed")) then
			...
			show("node0") -- It's easy to unlock the gate right now. We should ask for some card in upper level, maybe?
