 ...
 */
void put_miscellaneous_spell_effects(void)
{
	int i;
	...
	for (i = 0; i < all_spells.size; ++i) {
		...
		put_radial_blue_sparks(current_spell->spell_center.x, current_spell->spell_center.y,
							...
							current_spell->active_directions, current_spell->spell_age);
	}
}
...
void update_virtual_position(gps * target_pos, gps * source_pos, int level_num)
{
	...
	//
	if (source_pos->z < 0 || source_pos->z >= MAX_LEVELS ||
		level_num < 0 || level_num >= MAX_LEVELS) {
...
void AssembleCombatPicture(int mask)
{
	...
	put_miscellaneous_spell_effects();
	...
	if (!(mask & ONLY_SHOW_MAP_AND_TEXT)) {
		display_widgets();
		if (!world_frozen())
			show_texts_and_banner(GameOver);
	}
